...
export function EmailSignaturesPage({ user, bundles, signatures, guestSignature, initialSignatureId, saveState, saveError, onCreate, onUpdate, onGuestUpdate, onDuplicate, onDelete, onCards, onTemplates, onInsights, onSignatures, onBranding, onSignOut, onRequestAuth, onUploadAsset, onToast }: EmailSignaturesPageProps) {
  ...
  if (selected) return <><div className="signature-editor-workspace"><SignatureSidebar active="signatures" user={user} onCards={onCards} onTemplates={onTemplates} onInsights={onInsights} onSignatures={onSignatures} onBranding={onBranding} onSignOut={onSignOut} /><SignatureEditor signature={selected} bundles={bundles} saveState={saveState} saveError={saveError} mobilePreview={mobilePreview} onMobilePreview={setMobilePreview} onUpdate={onUpdate} onBack={() => setSelectedId(null)} onDuplicate={() => duplicate(selected)} onDelete={() => setConfirmDeleteId(selected.id)} onFinish={() => setReadyId(selected.id)} onUploadAsset={onUploadAsset} onToast={onToast} /></div>{confirmDeleteId && <DeleteSignatureDialog onCancel={() => setConfirmDeleteId(null)} onConfirm={() => void deleteSignature(confirmDeleteId)} />}</>
...
function DeleteSignatureDialog({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) {
  return <div className="dialog-backdrop signature-dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}><div className="confirm-dialog signature-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-signature-title" aria-describedby="delete-signature-description"><div className="confirm-dialog-icon"><Trash2 size={20} /></div><h2 id="delete-signature-title">Delete this signature?</h2><p id="delete-signature-description">This removes the signature from your workspace. Copied signatures already in your inboxes won’t be affected.</p><div className="confirm-dialog-actions"><button className="button button-ghost" onClick={onCancel}>Keep it</button><button className="button button-danger" onClick={onConfirm}>Delete signature</button></div></div></div>
}
